...
def cross_validate(val_loader, model):
    batch_time, losses, top1, top5 = [AverageMeter() for _ in range(4)]
    criterion = nn.CrossEntropyLoss()
    num_iter = len(val_loader)
    pbar = tqdm(range(num_iter))
...
def ensumble_validate(val_loader, models):
    batch_time, losses, top1, top5 = [AverageMeter() for _ in range(4)]
    criterion = nn.CrossEntropyLoss()
    num_iter = len(val_loader)
    pbar = tqdm(range(num_iter))
...
def adjust_learning_rate(optimizer, epoch, cfg):
    steps = np.sum(epoch > np.asarray(cfg.OPTIMIZER.LR_DECAY_STAGES))
    if steps > 0:
        new_lr = cfg.OPTIMIZER.LR * (cfg.OPTIMIZER.LR_DECAY_RATE**steps)
